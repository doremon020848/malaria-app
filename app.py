...
# ─── MASTER CSS (กูตัดไว้ให้สั้นลง แต่ใช้ CSS เดิมของมึงได้เลย) ──────────────────────
st.markdown("""
<style>
/* ... (ใส่ CSS เดิมของมึงทั้งหมดตรงนี้) ... */
...
:root { --ink: #04080f; --cyan: #00d2b4; --blue: #0a84ff; --red: #ff3d6b; --muted: #4d7a99; --txt: #d8eeff; --border: rgba(0,210,180,0.13); --border2: rgba(0,210,180,0.28); --r: 18px; }
...
.info-val { font-family: 'Space Mono', monospace; font-size: .95rem; color: var(--txt); font-weight: 700; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.info-key { font-size: .68rem; color: var(--muted); text-transform: uppercase; }
[data-testid="stButton"] > button { width: 100% !important; background: linear-gradient(135deg, var(--cyan) 0%, var(--blue) 100%) !important; color: #040c18 !important; font-family: 'Syne', sans-serif !important; font-weight: 700 !important; border-radius: 12px !important; padding: .9rem 2rem !important; }
...
try:
    ...
    st.error(f"⚠️ ไม่พบไฟล์ `{MODEL_PATH}` ในระบบ")
    st.stop()

# ─── SELECTION FROM SAMPLES ───────────────────────────────────────────────────
st.markdown('<div class="section-label">🔬 &nbsp;เลือกภาพตัวอย่างจากโฟลเดอร์ Samples</div>', unsafe_allow_html=True)
...
if os.path.exists(SAMPLE_DIR):
    ...
    if sample_images:
        selected_file = st.selectbox("เลือกภาพที่ต้องการวิเคราะห์:", sample_images)
        ...
        with col_info:
            ...
            <div class="info-grid">
                <div class="info-cell" style="grid-column:1/-1">
                    <div class="info-val">{selected_file}</div>
                ...
                <div class="info-cell" style="grid-column:1/-1">
                    ...
                    <div class="info-key">Input shape</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if run:
            ...
            if is_safe:
                ...
                box_cls, icon, title_th, title_en, pct_color = "result-safe", "✅", "ไม่ติดเชื้อ", "UNINFECTED · NORMAL", "#00d2b4"
            else:
                bar_html  = f'<div class="bar-fill-danger" style="width:{conf_pct:.1f}%"></div>'
                box_cls, icon, title_th, title_en, pct_color = "result-danger", "⚠️", "ตรวจพบเชื้อ", "INFECTED · MALARIA", "#ff3d6b"
            ...
            with st.expander("🔢 Raw model output"):
                st.code(f"sigmoid output = {prediction:.8f}\npredicted class = {'Uninfected' if is_safe else 'Infected'}", language="text")
    else:
        st.info("💡 เลือกรูปภาพจากเมนูด้านบน แล้วกดปุ่ม ANALYSE เพื่อเริ่มวิเคราะห์")
else:
