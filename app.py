...
# ─── DATA INPUT SECTION ──────────────────────────────────────────────────────
st.markdown('<p style="font-family:"Inter", sans-serif; font-size:1.2rem; margin-top:0px; font-weight:600;">SELECTION_MODE</p>', unsafe_allow_html=True)
...
if mode == "SAMPLES":
    if os.path.exists(SAMPLE_DIR):
        ...
        if files:
            st.markdown('<p style="font-family:\'Inter\', sans-serif; font-size:1rem; font-weight:500; color:#8892b0; margin-bottom:-10px;">CHOOSE DATASET:</p>', unsafe_allow_html=True)
            choice = st.selectbox(, files)
            if choice:
